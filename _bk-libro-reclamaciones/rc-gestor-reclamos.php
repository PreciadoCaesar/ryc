...
<head>
    ...
    <title>Panel de Reclamos | R&C Consulting</title>
    ...
    <div class="container">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-bordered bg-white mb-0 admin-table" id="tablaReclamos">
                ...
                <tbody>
                    <?php $i = 1; foreach ($reclamos as $r): ?>
                    <tr data-search="<?= htmlspecialchars(strtolower($r['codigo_seguimiento'] . ' ' . ($r['nombre_visible'] ?? ''))) ?>">
                        <td class="row-num text-center"><?= $i++ ?></td>
                        <td><a href="detalle-reclamo.php?codigo=<?= urlencode($r['codigo_seguimiento']) ?>" class="codigo-link"><?= htmlspecialchars($r['codigo_seguimiento']) ?></a></td>
                        <td><?= htmlspecialchars($r['nombre_visible'] ?? '-') ?></td>
                        <td><?= htmlspecialchars($r['doc_visible'] ?? '-') ?></td>
                        <td style="font-size:12px;"><?= date('d/m/Y', strtotime($r['fecha_creacion'])) ?></td>
                        <td>
                            <select class="estado-select estado-<?= str_replace(' ', '_', $r['estado']) ?>" data-codigo="<?= htmlspecialchars($r['codigo_seguimiento']) ?>" onchange="cambiarEstado(this)">
                        ...
                        <td class="text-center">
                            <?php if ($r['total_archivos'] > 0): ?>
                                <a href="detalle-reclamo.php?codigo=<?= urlencode($r['codigo_seguimiento']) ?>#archivos" class="badge-archivos text-decoration-none">
                        ...
                        <td class="text-center">
                            <a href="detalle-reclamo.php?codigo=<?= urlencode($r['codigo_seguimiento']) ?>" class="btn btn-sm btn-outline-primary">
                                <i class="bi bi-eye"></i>
                            </a>
                        </td>
                        <td class="text-center">
                            <a href="practica/generar_reclamo.php?codigo=<?= urlencode($r['codigo_seguimiento']) ?>&documento=<?= urlencode($r['tipo_persona'] === 'natural' ? ($r['doc_numero_natural'] ?? '') : ($r['ruc_juridica'] ?? '')) ?>" class="btn btn-sm btn-danger" target="_blank">
