...
## 11. Admin Dashboard Components (R&C Consulting)

### Course Cards & Miniaturas
- Use `image_promotion` field for course thumbnails on homepage
- Cards use shadow-as-border technique from Section 5
- Auto-loaded from database (replaces static `cursos.php`)
- Each course has an assigned `asesora_id` (advisor)

### Excel/DataTables for Leads
- Use Handsontable or native HTML tables with shadow borders
- Filter leads by advisor: `WHERE advisor_id = auth()->user()->advisor->id`
- Export to CSV/Excel with `maatwebsite/excel`
- Real-time updates via WebSockets (Pusher/Laravel Echo)
- Live indicator with pulse animation (see Section 6)

### Course Creation Form
- Upload field for `image_promotion` (thumbnail for homepage)
- Preview image before upload
- Assign advisor (asesora) to course
- Fields: title, subtitle, type (curso/diplomado), prices, dates, sessions
- Uses shadow-as-border on inputs (Section 5)
- Mobile-safe: input font-size ≥16px

### Advisor-Course-Lead Relationship
- Each advisor (asesora) is assigned to courses via `asesora_id`
- Leads automatically link to advisor via `advisor_id`
- Each advisor can only see their own leads in Excel view
- Dashboard stats: total courses, leads, advisors, closed sales

### Database Auto-Seed
- Migrations run with `php artisan migrate:fresh --seed`
- `AdvisorSeeder`: Creates advisors with WhatsApp/photo
- `CourseSeeder`: Creates courses with `image_promotion` thumbnails
- `LeadSeeder`: Creates sample leads linked to advisors
- Google OAuth fields: `google_id`, `avatar`, `rol` in users table
