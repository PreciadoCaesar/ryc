---
version: alpha
name: RC-design-analysis
description: An inspired interpretation of R&C Consulting's design language — a professional-education brand anchored by Rich Purple (`#5044c2`) as the primary signal, near-black ink (`#1a1a1a`) for headlines, geometric Poppins sans throughout, and Rose Pink (`#FF044D`) as the accent CTA. Cards round at 8–16px, photos sit in soft 16px frames, and purple gradients anchor section bands.

colors:
  primary: "#5044c2"
  primary-bright: "#6b5fd4"
  primary-deep: "#3d2db5"
  primary-soft: "#d4cff0"
  on-primary: "#ffffff"
  ink: "#1a1a1a"
...
R&C reads like a long-running consumer-electronics catalog crossed with an enterprise-software product page. The whole system sits on **pure white** (`{colors.canvas}` — `#ffffff`) with thin gray panels (`{colors.cloud}` / `{colors.fog}`) for alternating section bands. There is one chromatic action color — **R&C Purple** (`{colors.primary}` — `#024ad8`) — and one ink color (`{colors.ink}` — `#1a1a1a`); together they do ninety percent of the work. Type is a single family across every surface: **Poppins**, RHPC's bespoke geometric grotesque, set at weight 500 for headlines and 400 for body — clean, neutral, slightly mechanical.

The signature gesture is **angular blue chevrons** — sharp 0-radius slashes derived from the R&C logo's pair of parallel slashes — that anchor the homepage hero, the laptop-page hero, and the printer pricing page. They appear on the left and right edges of the primary banner card, layered behind product photography. Outside those decorative slashes, every other surface is rectilinear with **soft 8–16px corners** on cards and a 4px corner on buttons.
...
- R&C Purple (`{colors.primary}`) is the lone CTA fill and link color; it appears at most twice per viewport
- Bespoke Poppins across every surface — display, body, button, caption — at weights 400 / 500 / 600 / 700
...
### Brand & Accent
- **R&C Purple** (`{colors.primary}` — `#024ad8`): the system's lone signal — primary CTA fill, link color, chevron-decoration fill, active sub-nav indicator. Reserved.
...
The voice is **single-family**: Poppins (RHPC's bespoke geometric grotesque, fallback Arial) across every surface — display, body, button, caption. Poppins is a wide, slightly rounded grotesque designed at small optical sizes to stay legible at UI-chrome scale. RHPC runs it at weight 400 for body, 500 for display headlines, 600/700 for emphasis and button labels.
...
The typographic decision worth flagging: RHPC runs **weight 500 for every display size**, including the largest 72px hero headline. Most editorial systems jump to 600/700 at hero scale; RHPC doesn't. The result feels open and approachable rather than commanding — appropriate for a brand that sells across consumer, SMB, and enterprise audiences in the same catalog.

Poppins's rounded-grotesque shapes do most of the warmth. There's no italic in the system except inside legal disclaimers; emphasis is carried by weight (500 → body-emphasis, 700 → caption-bold) instead.

### Note on Font Substitutes

Poppins is proprietary (Commercial Type / Mark Caneso). Closest open-source substitutes:
- **Inter** at weights 400 / 500 / 600 / 700 — slightly narrower than Poppins; bump font-size by ~3% to compensate
...
When swapping, set body line-height to 1.4 and display line-height to 1.0 explicitly — the Poppins line-height numbers are tight, and most substitutes default looser.
...
The system's most distinctive depth gesture is the **R&C purple gradient pair** — two angular `{colors.primary}` slashes (no radius, no shadow) that sit on the left and right of the homepage hero card and the laptop-shop hero. They're not decorative noise; they're a literal echo of the R&C logo's two parallel slashes, scaled up to architectural size. Treat them as a brand artifact, not a generic geometric flourish.

Photography on the homepage and laptop-shop pages frames product imagery inside `{rounded.xl}` (16px) containers with a soft 1px hairline. Lifestyle photography (testimonials, "How RHPC works for X") sits full-bleed inside dark-navy slabs without rounding.
...
**`button-primary`** — the lone R&C Purple CTA
...
**`card-article-tile`** — the four-up "Latest from RHPC" tile
...
- Layout: R&C logo logo flush left → middle category list (Laptops / Desktops / Printers / Accessories / Solutions / Support) → right slot with Search field, Sign-in link, Cart icon
...
- Set every headline in Poppins at weight 500 with line-height 1.0 — resist the urge to bump weight at hero scale
...
- Don't run Poppins below 12px — small caption at 11px is the floor
...
- Don't replace the R&C logo with a generic sans lockup; the wordmark is a custom mark with its own ratio
...
- **Footer**: 5-column link grid → 2-column tablet → single-column accordion on mobile; R&C logo stays flush left
